...
class ThemeBase:
    ...
    icons = {
        ...
        'edit':        (_("Garden"),                "moin-edit.png",   12, 12),
...
""" % self._stylesheet_link(True, 'all', 'msie')
        ...
        webfont_css = render_template(
            '_webfont_css.html',
            request=self.request,
            is_mobile_preferred=request.user.mobile_preferred,
            is_mobile=re.search(r"android.+mobile|iphone|ipod", request.http_user_agent, re.I|re.M) is not None,
            is_mac=re.search(r"mac os x", request.http_user_agent, re.I|re.M) is not None)
    ...
    def pageinfo(self, page):
        ...
        if self.shouldShowPageinfo(page):
            ...
            if info:
                # meta = page._get_adpt().meta(self.request)
                meta = ''
                if info['editor']:
                ...
                html = '<p id="pageinfo" class="info"%(lang)s>%(info)s</p><div style="clear:both;"></div>%(meta)s\n' % {
                    'lang': self.ui_lang_attr(),
                    'info': info,
                    'meta': meta,
                    }

                # 1. 현재 페이지가 dayone소속이 아니라면
                # 2. 현재 storage중의 dayone목록을 찾아서
                # 3. 각 stroage의 tag를 검색
                # 4. 현재 페이지이름이 tag로 속해있으면 listup
                from MoinMoin.dayone_multi import DayoneMiddleware
                if self.request.user.include_dayone_backlinks and \
                        not isinstance(page.middleware, DayoneMiddleware):
                    # parser for markdown
                    markdown = wikiutil.importPlugin(self.request.cfg, 'parser', 'text_markdown', 'markdown')

                    # finding pages by tagname
                    pagenames = []
                    dayone_middlewares = filter(lambda x: isinstance(x,DayoneMiddleware), self.request.storage.values())
                    for middleware in dayone_middlewares:
                        for entry_pagename in middleware.filter_by_tag(self.request, page.page_name):
                            pagenames.append(entry_pagename)
                    # logging.critical(dayone_middlewares)
                    # logging.critical(pagenames)

                    # dayone template
                    dayone_html = []
                    pages = [Page(self.request, pn) for pn in pagenames]
                    pages.sort(key=lambda x: -x._get_adpt().created_at) # minus means the reverse
                    for entry_page in pages:
                        dayone_html.append(render_template('_dayone_body_inline.html',
                                page=entry_page,
                                pagename=entry_page.page_name,
                                request=self.request,
                                markdown=markdown))

                    html = u''.join([html, render_template('_pageinfo.html', request=self.request, page=page, htmls=dayone_html)])
...
''' % d
    ...
    def showversion(self, d, **keywords):
        ...
        if not keywords.get('print_mode', 0):
            html += render_template('_meta_detail.html', request=self.request)
        return html
...
#   });
# </script>''',
            u'<link href="%s/metronic/assets/global/plugins/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css"/>' % self.request.cfg.url_prefix_static,
            u'<link href="%s/metronic/assets/global/plugins/simple-line-icons/simple-line-icons.min.css" rel="stylesheet" type="text/css"/>' % self.request.cfg.url_prefix_static,
            u'<link href="%s/metronic/assets/global/plugins/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css"/>' % self.request.cfg.url_prefix_static,
            u'<link href="%s/metronic/assets/global/css/plugins.css" rel="stylesheet" type="text/css"/>' % self.request.cfg.url_prefix_static,
            u'<link href="%s/metronic/assets/global/css/components.css" rel="stylesheet" id="style_components" type="text/css" />' % self.request.cfg.url_prefix_static,
            # u'<link href="%s/metronic/assets/layouts/layout/css/layout.min.css" rel="stylesheet" type="text/css" />' % self.request.cfg.url_prefix_static,

            self.emit_custom_html(self.cfg.page_header1),
            # u'<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/css/bootstrap.min.css" integrity="sha384-PsH8R72JQ3SOdhVi3uxftmaW6Vc51MKb0q5P2rRUpPvrszuE4W1povHYgTpBfshb" crossorigin="anonymous">',
            self.externalScript('jquery-1.11.2.min'),
            self.externalScript('select2.min'),
            u'<script src="%s/metronic/assets/global/plugins/bootstrap/js/bootstrap.min.js" type="text/javascript"></script>' % self.request.cfg.url_prefix_static,
            # self.externalScript('d3.min'),
            u'<script src="/__moinfbp/sqlrun/static/js/sqlrun.js"></script>',
            u'<link href="%s/common/js/select2.min.css" rel="stylesheet" />' % self.request.cfg.url_prefix_static,
            u'<link href="%s/common/js/moin.css" rel="stylesheet" />' % self.request.cfg.url_prefix_static,
...
""" % {'url': page.url(self.request, querystr={'action': 'garden', 'editor': 'gui', }),
       'text': _('Garden (GUI)'),
      }
    ...
    def attachmentsLink(self, page):
        ...
        action_name = 'attachf' if self.request.user.use_attachf else 'AttachFile'

        return page.link_to(self.request,
                            text=_('Attachments'),
                            querystr={'action': action_name}, css_class='nbattachments', rel='nofollow')
    ...
    def startPage(self):
        ...
        """
        html = [
            render_template('_header.html', request=self.request),
    ...
    def endPage(self):
        ...
        html = [
            ...
            render_template('_footer.html', request=self.request),
        ]
        return u''.join(html)
...
""" % (
        ...
        # In standard mode, emit theme.header
            ...
            if keywords.get('editor_mode', 0):
                ...
                output.append(self.externalScript('moin'))
