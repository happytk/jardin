    ...
    def attachment_image(self, url, **kw):
        ...
        if exists:
            ...
            if fname.endswith('.mov') or fname.endswith('.mp4'):
                return self.video(**kw)
            elif fname.endswith('.mp3'):
                return self.audio(**kw)
...
''' % (ci, self._code_area_state[2], self._code_area_state[3],
    ...
    def audio(self, src=None, **kw):
        """Creates an inline audio with an <audio> element.

        The src argument must be the URL to the audio file.
        """
        if src:
            kw['src'] = src
            kw['type'] = 'audio/mpeg'
        return '<audio preload="auto" controls="controls">\n%s\nYour browser does not support HTML5 audio.\n</audio>' % self._open('source', **kw)


    def video(self, src=None, **kw):
        """Creates an inline video with an <video> element.

        The src argument must be the URL to the video file.
        """
        if src:
            kw['src'] = src
        return '<video controls autoplay="false">\n%s\nYour browser does not support HTML5 video.\n</video>' % self._open('source', **kw)
    ...
    def heading(self, on, depth, **kw):
        ...
        if not on:
            ...
            if self._allow_section_edit and not getattr(self.page, "print_mode", 0):
                ...
                querystring = u'%s?action=garden%s&srev=%d&startline=%d' % (sectioneditpage, backto, srev, self.lineno)
