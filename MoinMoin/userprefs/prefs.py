...
space between words. Group page name is not allowed.""", wiki=True) % wikiutil.escape(new_name)
        ...
        # try to get the (optional) webfont
        webfont_name = wikiutil.clean_input(form.get('webfont_name', self.cfg.webfont_default))
        if webfont_name != request.user.webfont_name:
            request.user.webfont_name = webfont_name

        # try to get the (optional) webfont
        mac_webfont_name = wikiutil.clean_input(form.get('mac_webfont_name', self.cfg.mac_webfont_default))
        if mac_webfont_name != request.user.mac_webfont_name:
            request.user.mac_webfont_name = mac_webfont_name

        # try to get the (optional) webfont
        mobile_webfont_name = wikiutil.clean_input(form.get('mobile_webfont_name', self.cfg.mobile_webfont_default))
        if mobile_webfont_name != request.user.mobile_webfont_name:
            request.user.mobile_webfont_name = mobile_webfont_name
        ...
        # user stylesheet
        request.user.usercss = form.get('usercss', '')
        request.user.mac_usercss = form.get('mac_usercss', '')
        request.user.mobile_usercss = form.get('mobile_usercss', '')
    ...
    def _webfont_select(self):
        """ create webfont selection. """
        import os
        MOIN_DIR = os.environ['MOIN']

        cur_webfont = self.request.user.valid and self.request.user.webfont_name or self.cfg.webfont_default
        options = [("<default>", "<%s>" % self._("Default"))]

        for f in sorted(os.listdir(os.path.join(MOIN_DIR, 'MoinMoinHtdocs', 'common', 'wf'))):
            if os.path.splitext(f)[-1].lower() in ['.eot', '.css', '.woff', 'woff2', '.otf', '.ttf']:
                options.append((f, f))
        return util.web.makeSelection('webfont_name', options, cur_webfont)

    def _mac_webfont_select(self):
        """ create webfont selection. """
        import os
        MOIN_DIR = os.environ['MOIN']

        cur_webfont = self.request.user.valid and self.request.user.mac_webfont_name or self.cfg.mac_webfont_default
        options = [("<default>", "<%s>" % self._("Default"))]

        for f in sorted(os.listdir(os.path.join(MOIN_DIR, 'MoinMoinHtdocs', 'common', 'wf'))):
            if os.path.splitext(f)[-1].lower() in ['.eot', '.css', '.woff', 'woff2', '.otf', '.ttf']:
                options.append((f, f))
        return util.web.makeSelection('mac_webfont_name', options, cur_webfont)

    def _mobile_webfont_select(self):
        """ create webfont selection. """
        import os
        MOIN_DIR = os.environ['MOIN']

        cur_webfont = self.request.user.valid and self.request.user.mobile_webfont_name or self.cfg.mobile_webfont_default
        options = [("<default>", "<%s>" % self._("Default"))]

        for f in sorted(os.listdir(os.path.join(MOIN_DIR, 'MoinMoinHtdocs', 'common', 'wf'))):
            if os.path.splitext(f)[-1].lower() in ['.eot', '.css', '.woff', 'woff2', '.otf', '.ttf']:
                options.append((f, f))
        return util.web.makeSelection('mobile_webfont_name', options, cur_webfont)
    ...
    def create_form(self):
        ...
        if request.user.valid:
                ...
                if not key in uf_remove:
                    if key in uf_disable:
                        self.make_row(_(label),
                                  [html.INPUT(**dict({'class': 'form-control', 'type':type, 'size':length, 'name':key, 'disabled':"disabled",
                                   'value':getattr(request.user, key)})), ' ', _(textafter), ])
                    else:
                        self.make_row(_(label),
                                  [html.INPUT(**dict({'class': 'form-control', 'type':type, 'size':length, 'name':key, 'value':getattr(request.user, key)})), ' ', _(textafter), ])
            ...
            if not self.cfg.theme_force and not "webfont_name" in self.cfg.user_form_remove:
                self.make_row(_('Preferred webfont'), [self._webfont_select()])
            if not self.cfg.theme_force and not "mac_webfont_name" in self.cfg.user_form_remove:
                self.make_row(_('Preferred mac webfont'), [self._mac_webfont_select()])
            if not self.cfg.theme_force and not "mobile_webfont_name" in self.cfg.user_form_remove:
                self.make_row(_('Preferred mobile webfont'), [self._mobile_webfont_select()])
            ...
            self.make_row(_('User CSS'), [
                html.TEXTAREA(name="usercss", rows="6", cols="50")
                    .append(request.user.usercss),
            ])

            self.make_row(_('User CSS(mac)'), [
                html.TEXTAREA(name="mac_usercss", rows="6", cols="50")
                    .append(request.user.mac_usercss),
            ])

            self.make_row(_('User CSS(mobile)'), [
                html.TEXTAREA(name="mobile_usercss", rows="6", cols="50")
                    .append(request.user.mobile_usercss),
            ])
        ...
        for name, label in buttons:
            if not name in self.cfg.user_form_remove:
                button_cell.extend([
                    html.INPUT(**{'type':"submit", 'name':name, 'value':label, 'class': 'btn btn-primary'}),
                    ' ',
                ])
