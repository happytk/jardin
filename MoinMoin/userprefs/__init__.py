...
class UserPrefBase(object):
    ...
    def make_form(self, explanation=None):
        ...
        _form = html.FORM(**{'action':action, 'class': 'form-horizontal form-bordered'})
        ...
        self._table = html.DIV(**{'class': 'form-body'})
    ...
    def make_row(self, label, cell, **kw):
        ...
        '''
        kw.update({'class': 'control-label col-md-3'})
        self._table.append(html.DIV(**{'class': 'form-group'}).extend([
            html.LABEL(**kw).extend([html.B().append(label), '   ']),
            html.DIV(**{'class': "col-md-9"}).extend(cell),
        ]))
