...
Please review the page and save then. Do not save this page as it is!""")
        ...
        request.write(unicode(html.INPUT(type="hidden", name="action", value="garden")))
        ...
        # button bar
        button_spellcheck = '<input type="submit" name="button_spellcheck" value="%s" onClick="flgChange = false;">' % _('Check Spelling')
...
If you don't want that, hit '''%(cancel_button_text)s''' to cancel your changes.""", wiki=True) % {
        ...
        request.write('''
<div class="btn-group">
<input class="btn btn-primary" type="submit" name="button_save" value="%s" onClick="flgChange = false;">
<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
    <i class="fa fa-angle-down"></i>
</button>
    <ul class="dropdown-menu" role="menu">
        <li>
            <input type="submit" name="button_preview" value="%s" onClick="flgChange = false;">
        </li>
''' % (save_button_text, _('Preview'), ))
        ...
        if not (request.cfg.editor_force and request.cfg.editor_default == 'text'):
            request.write('''
        <li>
<input id="switch2gui" style="display: none;" type="submit" name="button_switch" value="%s">
        </li>
''' % (_('GUI Mode'), ))
        ...
        if loadable_draft:
            request.write('''
        <li>
<input type="submit" name="button_load_draft" value="%s" onClick="flgChange = false;">
...
<input class="btn btn-danger" type="submit" name="button_cancel" value="%s">
...
<script type="text/javascript">
    ...
    //-->
        ...
        # request.write('<input type="text" name="ext">')
        request.write(
            u'''\
...
%(text)s\
        ...
        try:
            handler = wikiutil.importPlugin(self.request.cfg, "action", "attachf", "send_body")
            handler(self.page_name, self.request, editor=True)
...
Try a different name.""", wiki=True) % (wikiutil.escape(newpagename), )
    ...
    def saveText(self, newtext, rev, **kw):
        ...
        if not request.user.may.save(self, newtext, rev, **kw):
            msg = _('You are not allowed to garden this page!')
            raise self.AccessDenied(msg)
        elif not self.isWritable():
            msg = _('Page is immutable!')
...
Please review the page and save then. Do not save this page as it is!""")
        ...
        if not msg:
            ...
            # write the page file
            routed_pagename_fs = request.cfg.pagename_router(self.page_name_fs)
            # logging.warning(self.page_name_fs + '------' + routed_pagename_fs)
            routed_pagename = wikiutil.unquoteWikiname(routed_pagename_fs)
            Page.__init__(self, request, routed_pagename, ext=self.ext)
            if deleted or Page.is_write_file(self, newtext):
