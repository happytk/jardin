...
def execute(pagename, request):
    ...
    PageSummerEditor(request, pagename).sendEditor()


class PageSummerEditor(PageEditor.PageEditor):
...
Please review the page and save then. Do not save this page as it is!""")
        ...
        # send form
        request.write('''<form id="editor" method="post" action="%s#preview" onsubmit="$('#editor-textarea').val($('#summernote').code()); return true;">''' % (
        ...
        request.write('''<textarea id="editor-textarea" name="savetext" style='display:none;'></textarea>''')
...
''' % {
        ...
        path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'PageSummerEditor.html')
        ...
        def get_attachments(pagename, request, attach_dir):
            ...
            if files:
                ...
                for file in files:
                    ...
                    file = wikiutil.quoteWikinameFS(file)
        ...
        attach_dir = getAttachDir(request, self.page_name)
        files = get_attachments(self.page_name, request, attach_dir)
        request.write('<div id="summernote">%s</div>' % raw_body)
        request.write(template.render(pagename=self.page_name, \
                attachment_path=attach_dir, files=files, \
