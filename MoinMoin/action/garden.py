...
def execute(pagename, request):
    ...
    pg = Page(request, pagename, ext=ext)
    _pi = pg.pi
    ...
    if (request.cfg.edit_ticketing and
        ...
        request.theme.add_msg(_('Please use the interactive user interface to use action %(actionname)s!') % {'actionname': 'gardening' }, "error")
