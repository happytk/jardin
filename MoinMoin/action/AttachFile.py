...
def getAttachDir(request, pagename, create=0):
    ...
    return page._get_adpt().getAttachDir(check_create=create)
...
def _do_box(pagename, request):
    ...
    if if_modified and if_modified >= timestamp:
        ...
        filename_enc = filename.encode(request.cfg.attachment_charset)
    ...
    try:
        ...
        for zi in zf.infolist():
            ...
            if not name.endswith(zip_path_sep):  # a file (not a directory)
                ...
                finalname = finalname.decode(request.cfg.attachment_charset, 'replace')  # replaces trash with \uFFFD char
        ...
        if os.path.isdir(page_dir):
            ...
            for filename in files:
                ...
                data.addRow((
                    ...
                    # wikiutil.escape(filename.decode(request.cfg.attachment_charset)),
                    wikiutil.escape(wikiutil.unquoteWikiname(request.cfg.attachment_charset)),
