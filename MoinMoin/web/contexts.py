...
class BaseContext(Context):
    ...
    """ zeze-wecanfly customized feature
    """
    def storage(self):
        # if self.request.storage is not None:
        #     return self.request.storage
        storage = {}
        for k in self.cfg.middlewares:
            logging.debug('dispatch and create the WikiStorage - ' + k)
            middleware, args = self.cfg.middlewares[k]
            storage[k] = apply(middleware, args)
        return storage
    storage = EnvironProxy(storage)
