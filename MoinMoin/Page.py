...
def get_middleware_type(request, pagename):
    """
    get middleware-key(string) using given pagename
...
class WikiPage(object):
    ...
    def reset(self):
        ...
        # logging.warning(qpagename)
        # qpagename = pfmpagerouter.router(qpagename)
        qpagename = self.request.cfg.pagename_router(qpagename)
    ...
    def get_body(self):
        if self.__body is None:
            ...
            # try to open file
            try:
                # logging.warning(self._text_filename())
    ...
    def get_current_from_pagedir(self, pagedir):
        ...
        # if storage.exists(self):
        #     pagefile, rev, exists = storage.get_rev_dir(self)
        return rev
    ...
    def get_rev(self, use_underlay=-1, rev=0):
        ...
        if self._text_filename_force is None:
            ...
            if cache_data and (rev == 0 or rev == cache_data[1]):
                ...
                # logging.info("got data from cache: %r %r %r" % cache_data)
                return cache_data
    ...
    def _text_filename(self, **kw):
        ...
        # fname, rev, exists = storage.get_rev_dir(self)
        # print 'x'*100, fname
        return fname
    ...
    def parse_processing_instructions(self):
        ...
        middleware = get_middleware_type(request, self.page_name)
        if request.storage[middleware].format is None:
            pi['format'] = self.cfg.default_markup or "wiki"
        else:
            pi['format'] = request.storage[middleware].format
        logging.debug('%s %s format -> %s -> %s' % (middleware, self.page_name,  request.storage[middleware].format, pi['format']))
        pi['formatargs'] = ''
        pi['lines'] = len(meta)
        acl = []
    ...
    def makeCache(self, request, parser):
        ...
        try:
            ...
            # text = request.redirectedOutput(self.format, parser)
            # logging.error(text)
        finally:
...
# page router(mercurial, dayone, platfile, original wiki)
class Page(WikiPage):
    def __init__(self, request, page_name, **kw):
        WikiPage.__init__(self, request, page_name, **kw)
        # self.hgdb = request.hgdb
        self.__body = None
        self.padt = None
        self.middleware = None
        self.middleware_key = None
        self.ext = kw.get('ext', 'moin')

    # def middleware_type(self):
    #     """
    #     get middleware-key-string
    #     """
    ...
    @classmethod
    def get_middleware_type(cls, request, pagename):
        """
        get middleware-key(string) using given pagename
        you can middleware-object from cfg.middlewares[key][0]
        """
    ...
    def _get_adpt(self):
        ...
        if self.padt: return self.padt

        request = self.request
        pagename_fs = self.page_name_fs

        if hasattr(request, 'storage') and hasattr(request.cfg, 'routes') and hasattr(request.cfg, 'middlewares'):
            tester = request.cfg.routes.keys()
            for re_test in tester:
                if re_test.match(pagename_fs):
                    middleware_key = request.cfg.routes[re_test]
                    self.middleware_key = middleware_key
                    self.middleware = request.storage[middleware_key]
                    return self.middleware.get_adaptor(request, pagename_fs)

        #default
        from MoinMoin import storage
        self.padt = storage.MoinWikiPageAdaptor(request, pagename_fs)
        self.middleware_key = 'wiki'
        return self.padt
    ...
    def get_body(self):
        if self.rev is None or int(self.rev) == 0:
            if self.__body is None:

                text = self._get_adpt().get_body()

                if text is not None:
                    text = self.decodeTextMimeType(text)
                    self.__body = text
                else:
                    # todo: how about finding the text from file?
                    # raise Exception('Cannot fetch the body')
                    self.__body = ""
                # print 'x' * 100, self.rev
        else:
            if self.__body is None:
                # print 'z' * 100, self.rev
                self.__body= WikiPage.get_body(self)
                # print 'z' * 100, self.rev
        # PageO.get_body()
        return self.__body
    ...
    def exists(self, rev=0, domain=None, includeDeleted=False):
        # logging.warning(self._get_adpt().exists())
        return self._get_adpt().exists(rev, domain, includeDeleted)
    ...
    def is_write_file(self, newtext):
        ...
        caching.CacheEntry(self.request, arena, "pagelinks", scope='item').remove()

        newtext = self.encodeTextMimeType(newtext)
        return self._get_adpt().is_write_file(newtext)
    ...
    def isWritable(self):
        ...
        return self._get_adpt().isWritable()
    ...
    def send_raw(self, content_disposition=None, mimetype=None):
        ...
        if self.exists():
            ...
            request.last_modified = self._get_adpt().last_modified()
            text = self.encodeTextMimeType(self.body)
            #request.headers['Content-Length'] = len(text)  # XXX WRONG! text is unicode obj, but we send utf-8!
            if content_disposition:
                # TODO: fix the encoding here, plain 8 bit is not allowed according to the RFCs
    ...
    def delete_page(self):
        ...
        self._get_adpt().delete()

    def rename_page(self, new_pagename):
        """event use this function."""
        self._get_adpt().rename(new_pagename)

    # override
    # to prepend misc-data.
    def format(self, parser):
        """ Format and write page content without caching """
        self.request.write(self._get_adpt().header())
        parser.format(self.formatter)
        self.request.write(self._get_adpt().footer())
    ...
    def makeCache(self, request, parser):
        ...
        try:
            text = request.redirectedOutput(parser.format, formatter)
            text = self._get_adpt().header() + text + self._get_adpt().footer()
            # text = request.redirectedOutput(self.format, parser)
            # logging.error(text)
        finally:
            request.current_lang = saved_current_lang
    ...
    # override
    # to overwrite the link_text using page_adapter
    def link_to_raw(self, request, text, querystr=None, anchor=None, **kw):
        """ core functionality of link_to, without the magic """
        # logging.critical('%s > %s > %s > %s %s %s %s %s', text, querystr, anchor, kw, (not querystr or len(querystr) == 0), not anchor, text, self.page_name)
        if (not querystr or len(querystr) == 0 or kw.get('css_class',None) == 'backlink') and not anchor \
                          and text.replace(' ', '')[:3] == self.page_name[:3] \
                          and text.replace(' ', '')[-3:] == self.page_name[-3:]:
            try:
                overwritten_text = self._get_adpt().text()
                text = overwritten_text
            except AttributeError:
                pass
        # print text
        url = self.url(request, querystr, anchor=anchor, relative=True) # scriptName is added by link_tag
    ...
    def lastEditInfo(self, request=None):
        ...
        @return: timestamp and editor information
        """
        info = self._get_adpt().lastEditInfo(request)
        if not info:
            return WikiPage.lastEditInfo(self, request)
...
class RootPage(WikiRootPage):
    def __init__(self, request):
        WikiRootPage.__init__(self, request)
    ...
    def _listPages(self):
        ...
        # ret_pages = set()
        ret_pages = []
        for sn in request.storage:
            if request.cfg.all_pages_exclude_dayone:
                logging.debug('storage-check %s %s (is_dayone_instance:%s)', sn, request.storage[sn], isinstance(request.storage[sn], DayoneMiddleware))
                # logging.critical('storage-check %s %s %s', sn, request.storage[sn], isinstance(request.storage[sn], DayoneMiddleware))
                if isinstance(request.storage[sn], DayoneMiddleware): continue

            for pagename in request.storage[sn].list_pages(request):
                if get_middleware_type(request, pagename) == sn:
                    ret_pages.append(pagename)
            # pages = set(pages)
            # ret_pages = ret_pages.union(pages)

        # pages = self._listPageInPath(self.data_hg_dir)
        # if self.cfg.data_underlay_dir is not None:
        #     # Merge with pages from underlay
        #     path = self.getPagePath('pages', use_underlay=1)
        #     underlay = self._listPageInPath(path)
