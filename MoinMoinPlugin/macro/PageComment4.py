...
def _make_html(script_name, page_name, use_custum_author=False, row=3, entry_id=''):
    if use_custum_author:
        ...
        ret += "<tr><td width='70%'>"
        ret += "<textarea class='form-control' name='comment' id='gb_comment_for_%s' rows='%d'></textarea>" % (
            page_name, row)
        ret += "</td><td valign='bottom'><input type='text' name='custom_author' id='gb_author_for_%s' size='15' onfocus='this.value=\"\";'> <br/><input class='btn btn-primary' type='submit' value='comment'>" % page_name
...
//-->
        ...
        ret += "<textarea name='comment' class='form-control' rows='%d'></textarea>" % row
        ret += "<input class='btn btn-primary btn-block' type='submit' value='post'>"
        ret += "<input type='hidden' name='action' value='PageComment4'>"
        if use_custum_author:
            ret += "<input type='hidden' name='use_custum_author' value='%d'>" % use_custum_author
        if entry_id:
