...
def handle_git_file_attach(event):
    request = event.request
...
def handle_git_file_remove(event):
    request = event.request
    pagename = event.pagename
    filepath = os.path.join(getAttachDir(request, pagename), event.filename)

    middleware = get_middleware_type(request, pagename)
    request.storage[middleware].remove_attachment(request, pagename, filepath)


def handle(event):

    if isinstance(event, ev.FileAttachedEvent):
        handle_git_file_attach(event)
    elif isinstance(event, ev.FileRemovedEvent):
        handle_git_file_remove(event)
