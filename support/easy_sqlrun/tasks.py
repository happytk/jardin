...
from dbutil import dbconn, SqlRuntimeError
...
def sql_run_by_worker(sqlrunner_data):
    ...
    ## storage check
    SQLRUN_STORAGE = config.sqlrun_storage
    ...
    if SQLRUN_STORAGE == 'mongodb':
        # mongodb = spawn()
        ldb = config.sqlrun_mongodb_lockdb
        lcol = config.sqlrun_mongodb_lock_coll
...
class sqlrun(dict_obj):
    ...
    def __run(self):
        ...
        try:
            ...
            if self.dbname in config.sqlrun_dbconns:
                conn_str = config.sqlrun_dbconns[self.dbname]
                db = dbconn(conn_str)
            else:
                raise Exception()
